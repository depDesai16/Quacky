...
class SettingsPanelMixin:
    ...
    def _build_tab_security(self) -> QWidget:
        ...
        setup_title = QLabel("First-Run Checklist")
        setup_title.setObjectName("settingsRowLabel")
        setup_sub = QLabel("Check what still needs attention before you rely on Quacky day to day.")
        setup_sub.setObjectName("settingsRowSub")
        setup_sub.setWordWrap(True)
        self._settings_row_labels.extend([setup_title, setup_sub])
        card.add_widget(setup_title)
        card.add_widget(setup_sub)

        self._setup_status_label = QLabel("Loading setup status...")
        self._setup_status_label.setObjectName("settingsHint")
        self._setup_status_label.setWordWrap(True)
        self._settings_row_labels.append(self._setup_status_label)
        card.add_widget(self._setup_status_label)

        app_card = _SettingsCard()
        self._settings_cards.append(app_card)
        lay.addWidget(app_card)

        app_title = QLabel("Allowed App Control")
        app_title.setObjectName("settingsRowLabel")
        app_sub = QLabel("Choose exactly which apps or browser links Quacky may open.")
        app_sub.setObjectName("settingsRowSub")
        app_sub.setWordWrap(True)
        self._settings_row_labels.extend([app_title, app_sub])
        app_card.add_widget(app_title)
        app_card.add_widget(app_sub)

        app_controls = QWidget()
        self._app_controls_layout = QVBoxLayout(app_controls)
        self._app_controls_layout.setContentsMargins(0, 6, 0, 0)
        ...
        prefs_title = QLabel("Remembered Preferences")
        prefs_title.setObjectName("settingsRowLabel")
        prefs_sub = QLabel("Review or edit preferences Quacky has stored about how you like it to respond.")
        prefs_sub.setObjectName("settingsRowSub")
        prefs_sub.setWordWrap(True)
        self._settings_row_labels.extend([prefs_title, prefs_sub])
        prefs_card.add_widget(prefs_title)
        prefs_card.add_widget(prefs_sub)

        prefs_clear = QPushButton("Clear Preferences")
        prefs_clear.setObjectName("settingsActionButton")
        prefs_clear.setProperty("role", "danger")
        prefs_clear.setMinimumHeight(SETTINGS_METRICS["action_btn_h"])
        prefs_clear.clicked.connect(lambda: self._on_memory_clear("preferences"))
        self._settings_action_buttons.append(prefs_clear)
        prefs_card.add_widget(prefs_clear)

        prefs_container = QWidget()
        self._preferences_layout = QVBoxLayout(prefs_container)
        self._preferences_layout.setContentsMargins(0, 8, 0, 0)
        self._preferences_layout.setSpacing(8)
        prefs_card.add_widget(prefs_container)

        tasks_card = _SettingsCard()
        self._settings_cards.append(tasks_card)
        lay.addWidget(tasks_card)

        tasks_title = QLabel("Remembered Task Notes")
        tasks_title.setObjectName("settingsRowLabel")
        tasks_sub = QLabel("Edit or remove task notes and reminder-style memory Quacky has kept.")
        tasks_sub.setObjectName("settingsRowSub")
        tasks_sub.setWordWrap(True)
        self._settings_row_labels.extend([tasks_title, tasks_sub])
        tasks_card.add_widget(tasks_title)
        tasks_card.add_widget(tasks_sub)

        tasks_clear = QPushButton("Clear Task Notes")
        tasks_clear.setObjectName("settingsActionButton")
        tasks_clear.setProperty("role", "danger")
        tasks_clear.setMinimumHeight(SETTINGS_METRICS["action_btn_h"])
        tasks_clear.clicked.connect(lambda: self._on_memory_clear("tasks"))
        self._settings_action_buttons.append(tasks_clear)
        tasks_card.add_widget(tasks_clear)

        tasks_container = QWidget()
        self._tasks_layout = QVBoxLayout(tasks_container)
        self._tasks_layout.setContentsMargins(0, 8, 0, 0)
        self._tasks_layout.setSpacing(8)
        tasks_card.add_widget(tasks_container)
